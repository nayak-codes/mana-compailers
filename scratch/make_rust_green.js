...
const blogRustDir = path.join(publicDir, 'blog-rust');
const masterRustFile = path.join(publicDir, 'blog-rust.html');

// 1. Update public/blog-rust/style.css
const cssPath = path.join(blogRustDir, 'style.css');
let cssContent = `/* Specialized styling enhancements for Rust tutorial lessons & Accordion — Emerald Green Theme */
:root {
  --rust-theme: #10b981;
  --rust-theme-hover: #34d399;
  --rust-theme-bg: rgba(16, 185, 129, 0.12);
  --rust-theme-border: rgba(16, 185, 129, 0.3);
}
...
console.log('✅ Updated public/blog-rust/style.css to Emerald Green theme');

// Replace orange/rust colors in HTML files
function replaceRustColorsInFile(filePath) {
  let html = fs.readFileSync(filePath, 'utf8');

  // Replace orange/rust colors with Emerald Green
  html = html.replace(/#b7410e/g, '#10b981');
  html = html.replace(/#f97316/g, '#10b981');
  html = html.replace(/#d97706/g, '#059669');
  html = html.replace(/#ea580c/g, '#10b981');
  html = html.replace(/rgba\(183,\s*65,\s*14,/g, 'rgba(16, 185, 129,');
  html = html.replace(/rgba\(249,\s*115,\s*22,/g, 'rgba(16, 185, 129,');
  html = html.replace(/rgba\(217,\s*119,\s*6,/g, 'rgba(5, 150, 105,');

  fs.writeFileSync(filePath, html, 'utf8');
}

// 2. Update master index public/blog-rust.html
replaceRustColorsInFile(masterRustFile);
console.log('✅ Converted public/blog-rust.html to Emerald Green theme');

// 3. Update all 55 chapter HTML files in public/blog-rust/
const rustFiles = fs.readdirSync(blogRustDir).filter(f => f.endsWith('.html'));
let count = 0;
rustFiles.forEach(f => {
  replaceRustColorsInFile(path.join(blogRustDir, f));
  count++;
});

console.log(`🎉 Successfully converted all ${count} Rust chapter HTML files to Emerald Green theme!`);
